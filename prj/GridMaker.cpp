...
SquarePosition GridMaker::getPosition(const Pixel& pix) const{
	return { pix.height_pos / scale, pix.width_pos / scale };
}

void GridMaker::setState(const SquarePosition& position, const int state) {
	squares[position.hor][position.ver] = state;
}

int GridMaker::getState(const SquarePosition& position) const{
	return squares[position.hor][position.ver];
}
