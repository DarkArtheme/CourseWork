...
// Нужно адаптировать под изменение разрешения экрана
...
int main() {
	...
	//sf::RenderWindow window(sf::VideoMode(640, 360), "Prj-Towers");
...
bool manageGameCycle(sf::RenderWindow& window, const double scale_x, const double scale_y, int& level_number, int& clicked_level, bool& should_show_main_menu){
    ...
    //Загрузка уровня в класс, его обрабатывающий
    //std::vector<LevelMaker> lvlmaker;
    ...
    int type_ind = 0;
    //Основной цикл для отрисовки всех объектов и обработки событий
    GridMaker grid_maker(window, 16);
    int chosen_level = -1;
    sf::Clock clock;
    //sf::Clock clock_for_enemy;
    ...
    //enemies.emplace_back(BaseEnemy(lvlmaker, clock, grid_maker, sprite_side));
    ...
    while (window.isOpen()) {
        ...
        if(main_menu.isVisible()){
            ...
            //grid_maker.drawGrid(window);
            for (auto &button : tower_buttons) {
                button.draw(window);
            }

            //Начало этапа обработки попадания
            ...
            enemies = new_enemies;
            //updateQuery(towers, enemies);
            //конец этапа обработки попадания
...
//    buttons.push_back(std::make_shared<ButtonRec>(sf::FloatRect(button_left, button_top + period * 3, button_width, button_height)
//            , sf::Color(255, 120, 60, 255)
//            , sf::Color::Red, sf::Color::Red, sf::Color::White, font, 48, "Exit", 5));
